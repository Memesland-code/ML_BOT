...
export default async (channel: GuildChannel) => {
    ...
    try {
        ...
        await HandleLog(colors.red(`An error occured on ChannelDelete listener\n${error}`))
        await highLogsChannel.send({ content: `<@${botAdmins[0]}> An error occured on ChannelDelete listener\nPlease check console for full details` })
    }
}