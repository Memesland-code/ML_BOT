...
export default async (oldChannel: GuildChannel, newChannel: GuildChannel) => {
    ...
    try {
        ...
        await HandleLog(colors.red(`An error occured on ChannelUpdate listener\n${error}`))
        await highLogsChannel.send({ content: `<@${botAdmins[0]}> An error occured on ChannelUpdate listener\nPlease check console for full details` })
    }
}