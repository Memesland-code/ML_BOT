...
export default async (oldMember: GuildMember, newMember: GuildMember) => {
    ...
    try {
        ...
        await HandleLog(colors.red(`An error occured on GuildMemberUpdate listener\n${error}`))
        await highLogsChannel.send({ content: `<@${botAdmins[0]}> An error occured on GuildMemberUpdate listener\nPlease check console for full details` })
    }
}