...
const step6_button = new discord_js_1.ButtonBuilder()
    ...
    .setStyle(discord_js_1.ButtonStyle.Success);
const step6_1_button = new discord_js_1.ButtonBuilder()
...
const step6_1_row = new discord_js_1.ActionRowBuilder()
...
async function func_step6(interaction, confirmation5, response5, isError, collectorFilter) {
    var response6;
    if (isError) {
    ...
    try { //* Step 6 confirmation - vote
        ...
        const confirmation6 = await response6.awaitMessageComponent({ filter: collectorFilter, time: 600000 }); // 600_000
...
async function func_step6_1(interaction: any, confirmation6_1: any, response6_1: any, isError: boolean, collectorFilter: any) {
  ...
  */
async function func_step7(interaction, confirmation6, response6, isError, collectorFilter) {
    var response7;
    if (isError) {
    ...
    try { //* Step 7 confirmation - reveal impostor
        const confirmation7 = await response7?.awaitMessageComponent({ filter: collectorFilter, time: 600000 });
...
exports.default = {
    ...
    callback: async ({ interaction, args }) => {
        let interactor = interaction?.user.id;
        if (await (0, functions_1.IsBotPerformingMaintenance)() && !index_1.botAdmins.includes(interactor)) {
        ...
        step1.setFields({ name: "Joueur 1", value: `<@${args[0]}>` }, { name: "Joueur 2", value: `<@${args[1]}>` }, { name: "Joueur 3", value: `<@${args[2]}>` }, { name: "Joueur 4", value: `<@${args[3]}>` });
        var impostor = interaction?.client.users.cache.get(args[Math.floor(Math.random() * 4)]);
        if (args[6] == "true" && interaction?.channel?.type === discord_js_1.ChannelType.GuildText) {
                                    ...
                                    try { //* Step 4 confirmation - summary and start wait
                                        ...
                                        if (confirmation4.customId === "startgame") {
                                            ...
                                            try { //* Step 5 confirmation - in game
                                                ...
                                                const confirmation5 = await response5.awaitMessageComponent({ filter: collectorFilter, time: 900000 }); // 900_000
