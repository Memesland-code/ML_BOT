...
async function func_step6(interaction, confirmation5, response5, isError, collectorFilter) {
    ...
    try { //* Step 6 confirmation - vote
        ...
        step6.setFields({ name: "Temps restant", value: `<t:${Math.floor(voteTimestamp / 1000) + 6}:R>` }); // 600
        response6.edit({ embeds: [step6], components: [step6_row] });
        const confirmation6 = await response6.awaitMessageComponent({ filter: collectorFilter, time: 6000 }); // 600_000
...
async function func_step7(interaction, confirmation6, response6, isError, collectorFilter) {
    ...
    try { //* Step 7 confirmation - reveal impostor
        ...
        if (confirmation7.customId === "revealimpostor") {
            await confirmation7.update({ embeds: [step7_1], components: [step7_1_row] });
        }
    }
    catch (error) {
...
exports.default = {
    ...
    callback: async ({ interaction, args }) => {
        ...
        if (chosenMap == undefined)
            chosenMap = mapList[0];
        step2.setFields({ name: "", value: `${chosenMap}` });
                                    ...
                                    try { //* Step 4 confirmation - summary and start wait
                                        ...
                                        if (confirmation4.customId === "startgame") {
                                            ...
                                            try { //* Step 5 confirmation - in game
                                                ...
                                                step5.setFields({ name: "Temps restant", value: `<t:${Math.floor(startTimestamp / 1000) + 9}:R>` }); // 900
                                                confirmation4.editReply({ embeds: [step5], components: [step5_row] });
                                                const confirmation5 = await response5.awaitMessageComponent({ filter: collectorFilter, time: 9000 }); // 900_000
